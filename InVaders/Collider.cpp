	...
	if (intersectX < 0.0f && intersectY < 0.0f)
	{
		...
		else
		{
			if (delta_y > 0.0f)
			{
				...
				other.move(intersectY * push, 0.f);
...
bool Collider::checkCollision(Collider other, sf::Sprite& spriteOther, sf::Vector2f& direction, float push, bool& isIgnore)
{
	sf::Vector2f otherPosition = other.getPosition();
	sf::Vector2f otherHalfSize = other.getHalfSize();
	sf::Vector2f thisPosition = getPosition();
...
//Stair Collider
bool Collider::checkCollision(Collider other, sf::Sprite& spriteOther, sf::Vector2f& direction, float push, std::string type)
{
	sf::Vector2f otherPosition = other.getPosition();
	sf::Vector2f otherHalfSize = other.getHalfSize();
	sf::Vector2f thisPosition = getPosition();
	...
	if (intersectX < 0.0f && intersectY < 0.0f)
	{
		...
		if (intersectX > intersectY)
		{
			...
			{
				move(intersectX * (1.0f - push), 0.f);
				//if collide with the left side -> move up
				if (type == "stairR")
				{
					spriteOther.move(0.f, intersectX * push);
			...
			{
				move(-intersectX * (1.0f - push), 0.f);
				//if collide with the right side -> move up
				if (type == "stairL")
				{
					spriteOther.move(0.f, intersectX * push);
					other.move(0.f, intersectX * push);
